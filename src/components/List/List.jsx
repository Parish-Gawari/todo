    ...
    <div key={index}>
      ...
      >
          ...
          <>
            <div className="d-flex flex-column gap-1">
              <textarea
                ...
                className="p-1"
                onChange={(val) =>
                  itemListChangeHandler(index, val.target.value)
                }
              />
              <input
                type="date"
                ...
                className="p-1"
                onChange={(val) =>
                  itemListDateChangeHandler(index, val.target.value)
                }
              />
            </div>
        ...
        <div className="d-flex justify-content-evenly h-25 align-items-center">
          <input
            type="checkbox"
            onClick={() => isCheckHandler(index)}
            disabled={task.isDone === true}
          />
          <Button
            variant="transparent"
            className="border-none"
            onClick={() => isEditingHandler(index)}
            disabled={task.isDone === true}
            id={styles.Edit}
          >
            Edit
